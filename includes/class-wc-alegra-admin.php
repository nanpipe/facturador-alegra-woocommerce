...
class WC_Alegra_Admin {
            ...
            <form method="POST" action="options.php">
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td><input type="text" name="wc_alegra_email" value="<?php echo esc_attr(get_option('wc_alegra_email')); ?>" class="regular-text" /></td>
                    </tr>
                    <tr>
                        <th scope="row">Token API</th>
                        <td><input type="password" name="wc_alegra_token" value="<?php echo esc_attr(get_option('wc_alegra_token')); ?>" class="regular-text" /></td>
                    </tr>
                    <tr>
                        <th scope="row">ID impuesto exento</th>
                        <td>
                            <input type="number" name="wc_alegra_tax_exempt_id" value="<?php echo esc_attr(get_option('wc_alegra_tax_exempt_id', 1)); ?>" class="small-text" />
    ...
    public function ajax_process_invoice() {
        ...
        if ( ! $order ) wp_send_json_error('El pedido no existe.');
        if ( get_post_meta($order_id, 'alegra_invoice_id', true) ) wp_send_json_error('Este pedido ya tiene una factura generada.');
        ...
        if ( ! $client_id ) wp_send_json_error('No se pudo obtener el ID del cliente de Alegra.');
        ...
        if ( isset($response->id) ) {
            ...
            if ( isset($response->link) ) update_post_meta($order_id, 'alegra_invoice_url', esc_url_raw($response->link));
    ...
    public function ajax_delete_invoice_id() {
        ...
        $order_id = isset($_POST['order_id']) ? intval($_POST['order_id']) : 0;
        if ( ! $order_id ) wp_send_json_error('ID de pedido inválido.');
        ...
        if ( $deleted_id || $deleted_url ) {
             $order = wc_get_order($order_id);
             if ($order) $order->add_order_note("ID de factura Alegra borrado manualmente. Lista para regenerar.");
